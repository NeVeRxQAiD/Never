...
def change_nick_and_avatar_and_banner(server, nickname, token, avatar, banner):
    ...
    if nickname_changed and avatar_changed and banner_changed:
        print(f'{Fore.BLUE}[ Q9 ]{Fore.YELLOW} Token | {Fore.CYAN}{token_display} {Fore.GREEN} a and b and name is nice{Fore.RESET}')
    else:
        print(f'{Fore.RED}[ Q9 ] {Fore.YELLOW} Falid Error | {Fore.CYAN}{token_display}{Fore.RESET}')

def main():
    date_folder = 'python'

    
    config_path = os.path.join(date_folder, 'cobra.json')
    ...
    avatar_path = os.path.join(date_folder, 'Q9store.png')
    with open(avatar_path, "rb") as image_file:
        avatar = image_file.read()
    
    
    banner_path = os.path.join(date_folder, 'Q9storer.png')
    ...
    time.sleep(3)
    input(f'{Fore.MAGENTA}hi i`m cobra q9 team price any key for close | {Fore.RESET}')
