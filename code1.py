...
def rename(tk):
    ...
    if r.status_code in (200, 201, 204):
        print(Fore.GREEN +"[ Q9 ] Nice Name | %s" % (nickname))
        return "ok"
    else:
        print(Fore.RED +"[ Q9 ] Falid Nick Name Error")
        return "error"
...
def authorizer(tk):
    ...
    if r.status_code in (200, 201, 204):
        ...
        print("[ Q9 ] This Token Run")
        access_token = exchange['access_token']
        userid = get_user(access_token)
        add_to_guild(access_token, userid)
        print(Fore.GREEN + "[ Q9 ] This Token Join Server | %s" % (guild))
        if not nickname == "":
...
def main(tk):
    ...
    for x in idk:
        ...
        if r.status_code in (200, 201, 204):
            print(Fore.MAGENTA + "[ Q9 ] This Token Is Boosting %s" % (guild))
