...
class Main:
    ...
    def removeToken(token: str):
        with open('python/cobrax51.txt', "r") as f:
            Tokens = f.read().split("\n")
            for t in Tokens:
                if len(t) < 5 or t == token:
                    Tokens.remove(t)
            open("python/cobrax51.txt", "w").write("\n".join(Tokens))
...
def SORTER():
    ...
    tokens = Main().getAllTokens("python/cobrax51.txt")
    ...
    with open("python/cobrax51.txt", "w") as file: 
        file.truncate(0) 
        file.close()
    
    finished_time = int(datetime.now().strftime("%S"))
    elapsed = abs(start_time - finished_time)
    Main().sprint(f"[{color.RED}Hi I`m Cobra Q9 Team{color.RESET}] Complete | Time / {elapsed} | Total /{total_hits}")
    sleep(3)
